...
{	
	...
	if(empty($fname) || empty($lname) || empty($gender) || empty($birthdate) || empty($address) || empty($contact)) {
		...
		//updating the table
		$sql = "UPDATE tbl_player SET fname=:fname, lname=:lname, gender=:gender, birthdate=:birthdate, address=:address, contact=:contact WHERE studentid=:studentid";
...
$sql = "SELECT * FROM tbl_player WHERE studentid=:studentid";
$query = $dbConn->prepare($sql);
